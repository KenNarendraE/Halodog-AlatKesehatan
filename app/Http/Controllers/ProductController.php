...
class ProductController extends Controller
{
    public function index(Request $request)
    {
        ...
        $products = $query->orderBy('medical_devices.created_at', 'desc')->paginate(15);
    ...
    {
        $product = DB::table('medical_devices')
            ->leftJoin('device_types', 'medical_devices.type', '=', 'device_types.id')
            ->select('medical_devices.*', 'device_types.name as category_name')
            ->where('medical_devices.id', $id)
            ->first();
        ...
        return view('pages.product.show', compact('product', 'categories'));
    }
}
